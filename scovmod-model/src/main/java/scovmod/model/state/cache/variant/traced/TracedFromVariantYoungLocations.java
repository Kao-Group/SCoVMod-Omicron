...
import static scovmod.model.state.infection.InfectionState.TRACED_YOUNG;
import static scovmod.model.state.infection.InfectionState.TRACED_YOUNG_VARIANT;

public class TracedFromVariantYoungLocations implements Cache {

    IntSet locations = new IntOpenHashSet();

    public void wholeLocationUpdate(int locationId, LocalPopulation population) {
        if (!population.getAllInState(TRACED_YOUNG_VARIANT).isEmpty()) {
    ...
    public IntSet getFromVariantTracedYoungLocations() {
