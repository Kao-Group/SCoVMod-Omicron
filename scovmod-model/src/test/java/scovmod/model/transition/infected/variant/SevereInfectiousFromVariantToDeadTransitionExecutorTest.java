...
import scovmod.model.transition.infected.SevereInfectiousToDeadTransitionExecutor;
import scovmod.model.util.math.Random;

import static org.mockito.Mockito.*;

public class SevereInfectiousFromVariantToDeadTransitionExecutorTest {
...
//    DeathRates dr;
    ...
    public void youngPerson_CAN_Transition(){
        ...
        when(params.getSiToD_YoungRate_covidVariant()).thenReturn(SI_TO_D_RATE);
       // when(dr.getDeathRateModifierBasedOnHealthIndex(PERSON_1)).thenReturn(DEATH_RATE_MODIFICATION);

        SevereInfectiousFromVariantToDeadTransitionExecutor instance = new SevereInfectiousFromVariantToDeadTransitionExecutor(sm, rnd, TIMESTEP, params,slaac/*, dr*/);
    ...
    public void adultPerson_CAN_Transition(){
        ...
        when(slaac.getPeopleAgeClasses()).thenReturn(peopleAgeClasses);
        when(params.getSiToD_AdultRate_covidVariant()).thenReturn(SI_TO_D_RATE);
        //when(dr.getDeathRateModifierBasedOnHealthIndex(PERSON_1)).thenReturn(DEATH_RATE_MODIFICATION);
    ...
    public void elderlyPerson_CAN_Transition(){
        ...
        when(slaac.getPeopleAgeClasses()).thenReturn(peopleAgeClasses);
        when(params.getSiToD_ElderlyRate_covidVariant()).thenReturn(SI_TO_D_RATE);
       // when(dr.getDeathRateModifierBasedOnHealthIndex(PERSON_1)).thenReturn(DEATH_RATE_MODIFICATION);
