...
import static scovmod.model.state.infection.InfectionState.EXPOSED_ELDERLY_VARIANT;

public class ExposedToVariantElderlyLocations implements Cache {

    IntSet locations = new IntOpenHashSet();

    public void wholeLocationUpdate(int locationId, LocalPopulation population) {
        if (!population.getAllInState(EXPOSED_ELDERLY_VARIANT).isEmpty()) {
