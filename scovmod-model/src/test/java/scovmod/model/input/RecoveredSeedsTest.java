...
public class RecoveredSeedsTest {
	...
	public void loadData() throws URISyntaxException {
		Path testData = rootPath.resolve("areasToReseed.csv");
		RecoveredSeeds instance = new RecoveredSeeds(testData);
		...
		assertEquals(expectedAreaToReseedMap, instance.getAreaToReseedMap());
	}

	@Test(expected = BadInputDataException.class)
	public void badHeader() {
		new RecoveredSeeds(rootPath.resolve("areasToReseed_badHeader.csv"));
	}

//	@Test(expected = BadInputDataException.class)
//	public void exceptionIfRepeatedLocationId() {
//		Path repeatedRowFilePath = rootPath.resolve("areasToReseed_repeatedRow.csv");
//		new SeedsToReinfect(repeatedRowFilePath);
//	}
