...
public class BetaMatrixPhaseRate implements BetaRate { // Also known as second beta adjustment
    ...
    public double getBeta(InfectionState state, int location) {
        ...
        if (state == MILD_INFECTIOUS_YOUNG) {
            if (lrm.isDay()) return getSecondBetaChange(location, transModRatePerCA, this.betaYoungMild_day);
            else return getSecondBetaChange(location, transModRatePerCA, this.betaYoungMild_night);
        } else if (state == SEVERE_INFECTIOUS_YOUNG) {
            if (lrm.isDay()) return getSecondBetaChange(location, transModRatePerCA, this.betaYoungSevere_day);
            else return getSecondBetaChange(location, transModRatePerCA, this.betaYoungSevere_night);
        } else if (state == MILD_INFECTIOUS_ADULT) {
            if (lrm.isDay()) return getSecondBetaChange(location, transModRatePerCA, this.betaAdultMild_day);
            else return getSecondBetaChange(location, transModRatePerCA, this.betaAdultMild_night);
        } else if (state == SEVERE_INFECTIOUS_ADULT) {
            if (lrm.isDay()) return getSecondBetaChange(location, transModRatePerCA, this.betaAdultSevere_day);
            else return getSecondBetaChange(location, transModRatePerCA, this.betaAdultSevere_night);
        } else if (state == MILD_INFECTIOUS_ELDERLY) {
            if (lrm.isDay()) return getSecondBetaChange(location, transModRatePerCA, this.betaElderlyMild_day);
            else return getSecondBetaChange(location, transModRatePerCA, this.betaElderlyMild_night);
        } else if (state == MILD_INFECTIOUS_YOUNG_VARIANT) {
            if (lrm.isDay()) return getSecondBetaChange(location, transModRatePerCA, this.betaYoungMild_day_variant);
            else return getSecondBetaChange(location, transModRatePerCA, this.betaYoungMild_night_variant);
        } else if (state == SEVERE_INFECTIOUS_YOUNG_VARIANT) {
            if (lrm.isDay()) return getSecondBetaChange(location, transModRatePerCA, this.betaYoungSevere_day_variant);
            else return getSecondBetaChange(location, transModRatePerCA, this.betaYoungSevere_night_variant);
        } else if (state == MILD_INFECTIOUS_ADULT_VARIANT) {
            if (lrm.isDay()) return getSecondBetaChange(location, transModRatePerCA, this.betaAdultMild_day_variant);
            else return getSecondBetaChange(location, transModRatePerCA, this.betaAdultMild_night_variant);
        } else if (state == SEVERE_INFECTIOUS_ADULT_VARIANT) {
            if (lrm.isDay()) return getSecondBetaChange(location, transModRatePerCA, this.betaAdultSevere_day_variant);
            else return getSecondBetaChange(location, transModRatePerCA, this.betaAdultSevere_night_variant);
        } else if (state == MILD_INFECTIOUS_ELDERLY_VARIANT) {
            if (lrm.isDay()) return getSecondBetaChange(location, transModRatePerCA, this.betaElderlyMild_day_variant);
            else return getSecondBetaChange(location, transModRatePerCA, this.betaElderlyMild_night_variant);
        } else if (state == SEVERE_INFECTIOUS_ELDERLY_VARIANT) {
            if (lrm.isDay()) return getSecondBetaChange(location, transModRatePerCA, this.betaElderlySevere_day_variant);
            else return getSecondBetaChange(location, transModRatePerCA, this.betaElderlySevere_night_variant);
        } else {
            if (lrm.isDay()) return getSecondBetaChange(location, transModRatePerCA, this.betaElderlySevere_day);
            else return getSecondBetaChange(location, transModRatePerCA, betaElderlySevere_night);
        }
    }

    private double getSecondBetaChange(int location, double accessRateModPerCA, double beta) {
        return lrm.getSpecifiedBetaFactor(location) * params.getFirstBetaFactor() * beta
                * accessRateModPerCA * lrm.getLocalLockdownMultiplier(location);
    }
}
