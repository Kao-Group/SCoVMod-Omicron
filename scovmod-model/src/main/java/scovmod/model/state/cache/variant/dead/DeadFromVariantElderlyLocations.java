...
import static scovmod.model.state.infection.InfectionState.DEAD_ELDERLY_VARIANT;

public class DeadFromVariantElderlyLocations implements Cache {

    IntSet locations = new IntOpenHashSet();

    public void wholeLocationUpdate(int locationId, LocalPopulation population) {
        if (!population.getAllInState(DEAD_ELDERLY_VARIANT).isEmpty()) {
