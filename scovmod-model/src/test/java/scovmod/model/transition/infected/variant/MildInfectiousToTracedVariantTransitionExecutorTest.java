...
import scovmod.model.transition.infected.MildInfectiousToTracedTransitionExecutor;
import scovmod.model.util.math.Random;

import static org.mockito.Mockito.*;

public class MildInfectiousToTracedVariantTransitionExecutorTest {

    private static int PERSON_1 = 100;
    private static int LOCATION_1 = 1000;
    private static int TIMESTEP = 1;
    private static double MI_TO_T_RATE = 0.3;
    ...
    public void youngPerson_CAN_Transition(){
        ...
        when(params.getMiToT_YoungRate_covidVariant()).thenReturn(MI_TO_T_RATE);
        when(sq.getPersonLocation(PERSON_1)).thenReturn(LOCATION_1);
        when(tr.getTracingRate(MI_TO_T_RATE,LOCATION_1)).thenReturn(MI_TO_T_RATE);

        MildInfectiousToTracedVariantTransitionExecutor instance = new MildInfectiousToTracedVariantTransitionExecutor(sm, rnd, TIMESTEP, params,slaac, tr, sq);

        when(rnd.nextPoissonReturnsOneOrMore(MI_TO_T_RATE * TIMESTEP))
    ...
    public void adultPerson_CAN_Transition(){
        ...
        when(slaac.getPeopleAgeClasses()).thenReturn(peopleAgeClasses);
        when(params.getMiToT_AdultRate_covidVariant()).thenReturn(MI_TO_T_RATE);
        when(sq.getPersonLocation(PERSON_1)).thenReturn(LOCATION_1);
        when(tr.getTracingRate(MI_TO_T_RATE,LOCATION_1)).thenReturn(MI_TO_T_RATE);
    ...
    public void elderlyPerson_CAN_Transition(){
        ...
        when(slaac.getPeopleAgeClasses()).thenReturn(peopleAgeClasses);
        when(params.getMiToT_ElderlyRate_covidVariant()).thenReturn(MI_TO_T_RATE);
        when(sq.getPersonLocation(PERSON_1)).thenReturn(LOCATION_1);
        when(tr.getTracingRate(MI_TO_T_RATE,LOCATION_1)).thenReturn(MI_TO_T_RATE);
    ...
    public void elderlyPerson_CAN_Transition_InLocalLockdown(){
        ...
        when(params.getMiToT_ElderlyRate_covidVariant()).thenReturn(MI_TO_T_RATE);
        when(sq.getPersonLocation(PERSON_1)).thenReturn(LOCATION_1);
        when(tr.getTracingRate(MI_TO_T_RATE,LOCATION_1)).thenReturn(MI_TO_T_RATE*2);

        MildInfectiousToTracedVariantTransitionExecutor instance = new MildInfectiousToTracedVariantTransitionExecutor(sm, rnd, TIMESTEP, params,slaac, tr, sq);

        when(rnd.nextPoissonReturnsOneOrMore(MI_TO_T_RATE *2* TIMESTEP))
                .thenReturn(true, false);
