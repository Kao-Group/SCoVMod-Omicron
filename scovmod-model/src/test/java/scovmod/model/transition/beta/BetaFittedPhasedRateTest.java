...
public class BetaFittedPhasedRateTest {
    ...
    public void checkBetaRatesNotInLockdown_atNight() {
        BetaFittedPhasedRate instance = new BetaFittedPhasedRate(lrm, params,covidParams);
        when(lrm.isInLocalLockdown(LOCATION_1)).thenReturn(false);
        when(lrm.isDay()).thenReturn(false);
       // when(lrm.isInSpecifiedLockdown(LOCATION_1)).thenReturn(false);
        ...
        assertEquals(0.666*transMod, instance.getBeta(InfectionState.MILD_INFECTIOUS_YOUNG,LOCATION_1), TOL);
        assertEquals(1.2*transMod, instance.getBeta(InfectionState.SEVERE_INFECTIOUS_ELDERLY,LOCATION_1), TOL);
        assertEquals(0.666*transMod, instance.getBeta(InfectionState.MILD_INFECTIOUS_YOUNG_VARIANT,LOCATION_1), TOL);
        assertEquals(1.2*transMod, instance.getBeta(InfectionState.SEVERE_INFECTIOUS_ELDERLY_VARIANT,LOCATION_1), TOL);
    }
    ...
    public void checkBetaRatesNotInLockdown_duringDay() {
        ...
        when(lrm.getTansmissionModifierBasedOnTransmissionIndex(LOCATION_1)).thenReturn(transMod);
        when(lrm.getLocalLockdownMultiplier(LOCATION_1)).thenReturn(1.0);
        assertEquals(0.55*transMod, instance.getBeta(InfectionState.MILD_INFECTIOUS_YOUNG,LOCATION_1), TOL);
        assertEquals(1.0*transMod, instance.getBeta(InfectionState.SEVERE_INFECTIOUS_ELDERLY,LOCATION_1), TOL);
        assertEquals(0.55*transMod, instance.getBeta(InfectionState.MILD_INFECTIOUS_YOUNG_VARIANT,LOCATION_1), TOL);
        assertEquals(1.0*transMod, instance.getBeta(InfectionState.SEVERE_INFECTIOUS_ELDERLY_VARIANT,LOCATION_1), TOL);
    }
