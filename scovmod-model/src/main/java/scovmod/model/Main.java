...
import scovmod.model.output.modules.councilareaAllStates.CouncilAreaAllStatesSummaryResult;
...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        CovidVariantParameters covidParams = CovidVariantParameters.fromJSON(jsonConfig,modelParams);

        // Pull properly from json (in scala batch script)- only for test purposes
        String testModule = "caWithStates";

        Path rootDir = Paths.get("/home/ubuntu/tbmi/results/");
        ...
        switch (testModule) {
            ...
            case "ca":
                ...
                break;
            case "caWithStates":
                out = new scovmod.model.output.modules.councilareaAllStates.CouncilAreaAllStatesSummary();
                break;
//            case "nationalTest":
...
//        IResult result = Factory.buildModel(inputData, configParams, modelParams, covidParams, out).run();
...
//        }
//        writeData(Paths.get("/home/ubuntu/_CODE/scovmod-model/test.csv"),"date,area,measurement,value",lines);

        IResult result = Factory.buildModel(inputData, configParams, modelParams, covidParams, out).run();
        CouncilAreaAllStatesSummaryResult csr = (CouncilAreaAllStatesSummaryResult) result;
        List<String> lines = new ArrayList<>();
        for (Map.Entry<scovmod.model.output.modules.councilareaAllStates.DailyResult, Integer> kvp : csr.getSimulatedDailyResultPerCouncilArea().entrySet()) {
