...
import static scovmod.model.state.infection.InfectionState.HOSPITALISED_ELDERLY_VARIANT;

public class HospitalisedFromVariantElderlyLocations implements Cache {

    IntSet locations = new IntOpenHashSet();

    public void wholeLocationUpdate(int locationId, LocalPopulation population) {
        if (!population.getAllInState(HOSPITALISED_ELDERLY_VARIANT).isEmpty()) {
    ...
    public IntSet getHospitalisedFromVariantElderlyLocations() {
