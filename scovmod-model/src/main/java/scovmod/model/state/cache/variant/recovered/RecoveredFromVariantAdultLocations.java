...
import static scovmod.model.state.infection.InfectionState.RECOVERED_ADULT_VARIANT;

public class RecoveredFromVariantAdultLocations implements Cache {

    IntSet locations = new IntOpenHashSet();

    public void wholeLocationUpdate(int locationId, LocalPopulation population) {
        if (!population.getAllInState(RECOVERED_ADULT_VARIANT).isEmpty()) {
