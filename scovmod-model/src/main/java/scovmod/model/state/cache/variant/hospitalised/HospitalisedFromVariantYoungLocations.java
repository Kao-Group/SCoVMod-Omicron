...
import static scovmod.model.state.infection.InfectionState.HOSPITALISED_YOUNG_VARIANT;

public class HospitalisedFromVariantYoungLocations implements Cache {

    IntSet locations = new IntOpenHashSet();

    public void wholeLocationUpdate(int locationId, LocalPopulation population) {
        if (!population.getAllInState(HOSPITALISED_YOUNG_VARIANT).isEmpty()) {
