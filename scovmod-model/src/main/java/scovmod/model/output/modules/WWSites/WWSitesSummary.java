...
public class WWSitesSummary extends OutputModuleAdapter {
    
    private final Map<DailyResult, Double> dailyDZResult = new HashMap<>();
    ...
    public IResult buildResult() {
        return new WWSitesSummaryResult(dailyDZResult);
    }

//    @Override
//    public void newMildInfectiousDZ(LocalDate date, int dz, int personID) {
//        if(dz!=0) {
//            DailyEvent de = new DailyEvent(personID, WWSitesValueType.NEW_MILD_INFECTIOUS, dz);
...
//            String dz = dailyEvent.getSiteID();
...
//            String dz = dailyResult.getSiteID();
...
*/
    ...
    public void currentTotalRNASignal(LocalDate date, double value, String site) {
        if(site!="") {
            DailyResult ar = new DailyResult(date, WWSitesValueType.RNA_SIGNAL, site);
            incrementDailyResult(ar, value);
        }
    }
    ...
    private void incrementDailyResult(DailyResult ar, double value) {
        if (dailyDZResult.containsKey(ar)) {
            double currentCount = dailyDZResult.get(ar);
