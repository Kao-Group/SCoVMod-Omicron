...
public class SevereInfectiousFromVariantToDeadTransitionExecutor {
   ...
   // private final DeathRates dr;
    ...
    public SevereInfectiousFromVariantToDeadTransitionExecutor(
            StateModifier sm,
            Random rnd,
            double timeStep,
    ...
    public void apply(int personID) {
       ...
       // double deathRateModPerHB = dr.getDeathRateModifierBasedOnHealthIndex(personID);
            ...
            switch(ageClass){
                case YOUNG:
                    if (rnd.nextPoissonReturnsOneOrMore(params.getSiToD_YoungRate_covidVariant() /** deathRateModPerHB*/ * timeStep)) {
                        sm.updateInfectionState(personID, DEAD_YOUNG_VARIANT);
                    }
                    break;
                case ADULT:
                    if (rnd.nextPoissonReturnsOneOrMore(params.getSiToD_AdultRate_covidVariant()/* * deathRateModPerHB*/ * timeStep)) {
                        sm.updateInfectionState(personID, DEAD_ADULT_VARIANT);
                    }
                    break;
                case ELDERLY:
                    if (rnd.nextPoissonReturnsOneOrMore(params.getSiToD_ElderlyRate_covidVariant()/* * deathRateModPerHB*/ * timeStep)) {
