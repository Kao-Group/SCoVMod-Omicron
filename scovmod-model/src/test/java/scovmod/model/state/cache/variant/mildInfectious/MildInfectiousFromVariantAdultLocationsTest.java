...
public class MildInfectiousFromVariantAdultLocationsTest {
	...
	public void updatingByWholeLocationStatus(){
		MildInfectiousFromVariantAdultLocations instance = new MildInfectiousFromVariantAdultLocations();
		...
		assertEquals(NONE, instance.getMildInfectiousFromVariantAdultLocations());

		FakeLocalPopulation location2pop = new FakeLocalPopulation();
		location2pop.testSet(PERSON_5, MILD_INFECTIOUS_ADULT_VARIANT);
		instance.wholeLocationUpdate(LOCATION_2, location2pop);

		assertEquals(setOf(LOCATION_2), instance.getMildInfectiousFromVariantAdultLocations());

		FakeLocalPopulation location1updatedPop = new FakeLocalPopulation();
		location1updatedPop.testSet(PERSON_1, SUSCEPTIBLE);
		location1updatedPop.testSet(PERSON_3,MILD_INFECTIOUS_ADULT_VARIANT);
		instance.wholeLocationUpdate(LOCATION_1, location1updatedPop);

		assertEquals(setOf(LOCATION_2, LOCATION_1), instance.getMildInfectiousFromVariantAdultLocations());
	}
	...
	public void edgeCaseUpdatesDoNothing(){
		...
		FakeLocalPopulation location1pop = new FakeLocalPopulation();
		location1pop.testSet(PERSON_1, MILD_INFECTIOUS_ADULT_VARIANT);
		location1pop.testSet(PERSON_2, EXPOSED_YOUNG);
		...
		assertEquals(setOf(LOCATION_1), instance.getMildInfectiousFromVariantAdultLocations());
	}
