...
public class ExposedToTracedVariantTransitionExecutor {
    ...
    public ExposedToTracedVariantTransitionExecutor(
            StateModifier sm,
            Random rnd,
            double timeStep,
    ...
    public void apply(int personID) {
        ...
        int location = sq.getPersonLocation(personID);
            ...
            switch(ageClass){
                case YOUNG:
                    if (rnd.nextPoissonReturnsOneOrMore(tr.getTracingRate(params.geteToT_youngRate_covidVariant(),location) * timeStep)) {
                        sm.updateInfectionState(personID, TRACED_YOUNG_VARIANT);
                    }
                    break;
                case ADULT:
                    if (rnd.nextPoissonReturnsOneOrMore(tr.getTracingRate(params.geteToT_adultRate_covidVariant(),location) * timeStep)) {
                        sm.updateInfectionState(personID, TRACED_ADULT_VARIANT);
                    }
                    break;
                case ELDERLY:
                    if (rnd.nextPoissonReturnsOneOrMore(tr.getTracingRate(params.geteToT_elderlyRate_covidVariant(),location) * timeStep)) {
