...
import scovmod.model.transition.infected.TracedToDeadTransitionExecutor;
import scovmod.model.util.math.Random;

import static org.mockito.Mockito.*;

public class TracedToDeadFromVariantTransitionExecutorTest {

    private static int PERSON_1 = 100;
    private static int TIMESTEP = 1;
    private static double T_TO_D_RATE = 0.3;
...
//    DeathRates dr;
    ...
    public void youngPerson_CAN_Transition(){
        ...
        when(params.gettToD_YoungRate_covidVariant()).thenReturn(T_TO_D_RATE);
       // when(dr.getDeathRateModifierBasedOnHealthIndex(PERSON_1)).thenReturn(DEATH_RATE_MODIFICATION);

        TracedToDeadFromVariantTransitionExecutor instance = new TracedToDeadFromVariantTransitionExecutor(sm, rnd, TIMESTEP, params,slaac/*, dr*/);

        when(rnd.nextPoissonReturnsOneOrMore(T_TO_D_RATE /** DEATH_RATE_MODIFICATION*/ * TIMESTEP))
                .thenReturn(true, false);

        instance.apply(PERSON_1);

        verify(sm).updateInfectionState(PERSON_1, InfectionState.DEAD_YOUNG_VARIANT);
    ...
    public void adultPerson_CAN_Transition(){
        ...
        when(slaac.getPeopleAgeClasses()).thenReturn(peopleAgeClasses);
        when(params.gettToD_AdultRate_covidVariant()).thenReturn(T_TO_D_RATE);
       // when(dr.getDeathRateModifierBasedOnHealthIndex(PERSON_1)).thenReturn(DEATH_RATE_MODIFICATION);
        ...
        verify(sm).updateInfectionState(PERSON_1, InfectionState.DEAD_ADULT_VARIANT);
        verifyNoMoreInteractions(sm);
    }
    ...
    public void elderlyPerson_CAN_Transition(){
        ...
        when(slaac.getPeopleAgeClasses()).thenReturn(peopleAgeClasses);
        when(params.gettToD_ElderlyRate_covidVariant()).thenReturn(T_TO_D_RATE);
       // when(dr.getDeathRateModifierBasedOnHealthIndex(PERSON_1)).thenReturn(DEATH_RATE_MODIFICATION);
        ...
        verify(sm).updateInfectionState(PERSON_1, InfectionState.DEAD_ELDERLY_VARIANT);
        verifyNoMoreInteractions(sm);
    }
