...
import static scovmod.model.state.infection.InfectionState.HOSPITALISED_ADULT_VARIANT;

public class HospitalisedFromVariantAdultLocations implements Cache {

    IntSet locations = new IntOpenHashSet();

    public void wholeLocationUpdate(int locationId, LocalPopulation population) {
        if (!population.getAllInState(HOSPITALISED_ADULT_VARIANT).isEmpty()) {
