...
public class MildInfectiousToSevereVariantTransitionExecutor {
    ...
    public MildInfectiousToSevereVariantTransitionExecutor(
            StateModifier sm,
            Random rnd,
            double timeStep,
    ...
    public void apply(int personID) {
        AgeClass ageClass = peopleAgeClasses.getOrDefault(personID,AgeClass.YOUNG);
            ...
            switch(ageClass){
                case YOUNG:
                    if (rnd.nextPoissonReturnsOneOrMore(params.getMiToSI_YoungRate_covidVariant() * timeStep)) {
                        sm.updateInfectionState(personID, SEVERE_INFECTIOUS_YOUNG_VARIANT);
                    }
                    break;
                case ADULT:
                    if (rnd.nextPoissonReturnsOneOrMore(params.getMiToSI_AdultRate_covidVariant() * timeStep)) {
                        sm.updateInfectionState(personID, SEVERE_INFECTIOUS_ADULT_VARIANT);
                    }
                    break;
                case ELDERLY:
                    if (rnd.nextPoissonReturnsOneOrMore(params.getMiToSI_ElderlyRate_covidVariant() * timeStep)) {
                        sm.updateInfectionState(personID, SEVERE_INFECTIOUS_ELDERLY_VARIANT);
                    }
                    break;
                default:
