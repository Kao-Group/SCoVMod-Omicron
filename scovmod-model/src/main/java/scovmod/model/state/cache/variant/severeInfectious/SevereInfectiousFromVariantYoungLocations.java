...
import static scovmod.model.state.infection.InfectionState.SEVERE_INFECTIOUS_YOUNG_VARIANT;

public class SevereInfectiousFromVariantYoungLocations implements Cache {

    IntSet locations = new IntOpenHashSet();

    public void wholeLocationUpdate(int locationId, LocalPopulation population) {
        if (!population.getAllInState(SEVERE_INFECTIOUS_YOUNG_VARIANT).isEmpty()) {
