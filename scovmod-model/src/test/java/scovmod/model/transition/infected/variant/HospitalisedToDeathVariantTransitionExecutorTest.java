...
import scovmod.model.transition.infected.HospitalisedToDeadTransitionExecutor;
import scovmod.model.util.math.Random;

import static org.mockito.Mockito.*;

public class HospitalisedToDeathVariantTransitionExecutorTest {
...
//    DeathRates dr;
    ...
    public void youngPerson_CAN_Transition(){
        ...
        when(params.gethToD_YoungRate_covidVariant()).thenReturn(H_TO_D_RATE);
        //when(dr.getDeathRateModifierBasedOnHealthIndex(PERSON_1)).thenReturn(DEATH_RATE_MODIFICATION);

        HospitalisedToDeadVariantTransitionExecutor instance = new HospitalisedToDeadVariantTransitionExecutor(sm, rnd, TIMESTEP, params,slaac/*, dr*/);
    ...
    public void adultPerson_CAN_Transition(){
        ...
        when(slaac.getPeopleAgeClasses()).thenReturn(peopleAgeClasses);
        when(params.gethToD_AdultRate_covidVariant()).thenReturn(H_TO_D_RATE);
        //when(dr.getDeathRateModifierBasedOnHealthIndex(PERSON_1)).thenReturn(DEATH_RATE_MODIFICATION);
    ...
    public void elderlyPerson_CAN_Transition(){
        ...
        when(slaac.getPeopleAgeClasses()).thenReturn(peopleAgeClasses);
        when(params.gethToD_ElderlyRate_covidVariant()).thenReturn(H_TO_D_RATE);
        //when(dr.getDeathRateModifierBasedOnHealthIndex(PERSON_1)).thenReturn(DEATH_RATE_MODIFICATION);
