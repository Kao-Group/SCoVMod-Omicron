...
public class TracedToRecoveredTransitionExecutor {
    ...
    public TracedToRecoveredTransitionExecutor(
            StateModifier sm,
            Random rnd,
            double timeStep,
            Parameters params,
    ...
    public void apply(int personID) {
        AgeClass ageClass = peopleAgeClasses.getOrDefault(personID,AgeClass.YOUNG);
            ...
            switch(ageClass){
                case YOUNG:
                    if (rnd.nextPoissonReturnsOneOrMore(params.gettToR_YoungRate() * timeStep)) {
                        sm.updateInfectionState(personID, RECOVERED_YOUNG);
                    }
                    break;
                case ADULT:
                    if (rnd.nextPoissonReturnsOneOrMore(params.gettToR_AdultRate() * timeStep)) {
                        sm.updateInfectionState(personID, RECOVERED_ADULT);
                    }
                    break;
                case ELDERLY:
                    if (rnd.nextPoissonReturnsOneOrMore(params.gettToR_ElderlyRate() * timeStep)) {
