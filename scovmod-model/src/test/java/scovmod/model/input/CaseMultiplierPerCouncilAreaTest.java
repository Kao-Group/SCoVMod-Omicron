...
public class CaseMultiplierPerCouncilAreaTest {

	private Path rootPath = Paths.get("src", "test", "resources", "inputData", "caseMultiplierPerCouncilArea");

	@Test
	public void loadData() throws URISyntaxException {
		Path testData = rootPath.resolve("caseMultiplier.csv");
		CaseMultiplierPerCouncilArea instance = new CaseMultiplierPerCouncilArea(testData);
		...
		assertEquals(expectedIndex, instance.getCaseMultiplierPerCA());
	}

	@Test(expected = BadInputDataException.class)
	public void badHeader() {
		new CaseMultiplierPerCouncilArea(rootPath.resolve("caseMultiplier_badHeader.csv"));
	}

	@Test(expected = BadInputDataException.class)
	public void exceptionIfRepeatedLocationId() {
		Path repeatedRowFilePath = rootPath.resolve("caseMultiplier_repeatedRow.csv");
		new CaseMultiplierPerCouncilArea(repeatedRowFilePath);
	}
