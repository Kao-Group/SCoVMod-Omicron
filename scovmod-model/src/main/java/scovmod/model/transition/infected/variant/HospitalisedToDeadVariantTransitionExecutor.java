...
public class HospitalisedToDeadVariantTransitionExecutor {
   ...
   // private final DeathRates dr;
    ...
    public HospitalisedToDeadVariantTransitionExecutor(
            StateModifier sm,
            Random rnd,
            double timeStep,
    ...
    public void apply(int personID) {
       ...
       // double deathRateModPerHB = dr.getDeathRateModifierBasedOnHealthIndex(personID);
        ...
        //Get what age group the person is in
            switch(ageClass){
                case YOUNG:
                    if (rnd.nextPoissonReturnsOneOrMore(params.gethToD_YoungRate_covidVariant() /** deathRateModPerHB*/ * timeStep)) {
                        sm.updateInfectionState(personID, DEAD_YOUNG_VARIANT);
                    }
                    break;
                case ADULT:
                    if (rnd.nextPoissonReturnsOneOrMore(params.gethToD_AdultRate_covidVariant() /** deathRateModPerHB*/ * timeStep)) {
                        sm.updateInfectionState(personID, DEAD_ADULT_VARIANT);
                    }
                    break;
                case ELDERLY:
                    if (rnd.nextPoissonReturnsOneOrMore(params.gethToD_ElderlyRate_covidVariant() /** deathRateModPerHB*/ * timeStep)) {
