...
import static scovmod.model.state.infection.InfectionState.TRACED_ELDERLY_VARIANT;

public class TracedFromVariantElderlyLocations implements Cache {

    IntSet locations = new IntOpenHashSet();

    public void wholeLocationUpdate(int locationId, LocalPopulation population) {
        if (!population.getAllInState(TRACED_ELDERLY_VARIANT).isEmpty()) {
    ...
    public IntSet getTracedFromVariantElderlyLocations() {
