...
public class TracedToDeadTransitionExecutor {
   ...
   // private final DeathRates dr;
    ...
    public TracedToDeadTransitionExecutor(
            StateModifier sm,
            Random rnd,
            double timeStep,
            Parameters params,
    ...
    public void apply(int personID) {
        ...
        //double deathRateModPerHB = dr.getDeathRateModifierBasedOnHealthIndex(personID);
            ...
            switch(ageClass){
                case YOUNG:
                    if (rnd.nextPoissonReturnsOneOrMore(params.gettToD_YoungRate() /** deathRateModPerHB*/ * timeStep)) {
                        sm.updateInfectionState(personID, DEAD_YOUNG);
                    }
                    break;
                case ADULT:
                    if (rnd.nextPoissonReturnsOneOrMore(params.gettToD_AdultRate() /** deathRateModPerHB*/ * timeStep)) {
                        sm.updateInfectionState(personID, DEAD_ADULT);
                    }
                    break;
                case ELDERLY:
                    if (rnd.nextPoissonReturnsOneOrMore(params.gettToD_ElderlyRate() /** deathRateModPerHB*/ * timeStep)) {
