...
public class MildInfectiousToRecoveredVariantTransitionExecutor {
    ...
    public MildInfectiousToRecoveredVariantTransitionExecutor(
            StateModifier sm,
            Random rnd,
            double timeStep,
    ...
    public void apply(int personID) {
        AgeClass ageClass = peopleAgeClasses.getOrDefault(personID,AgeClass.YOUNG);
            ...
            switch(ageClass){
                case YOUNG:
                    if (rnd.nextPoissonReturnsOneOrMore(params.getMiToR_YoungRate_covidVariant() * timeStep)) {
                        sm.updateInfectionState(personID, RECOVERED_YOUNG_VARIANT);
                    }
                    break;
                case ADULT:
                    if (rnd.nextPoissonReturnsOneOrMore(params.getMiToR_AdultRate_covidVariant() * timeStep)) {
                        sm.updateInfectionState(personID, RECOVERED_ADULT_VARIANT);
                    }
                    break;
                case ELDERLY:
                    if (rnd.nextPoissonReturnsOneOrMore(params.getMiToR_ElderlyRate_covidVariant() * timeStep)) {
