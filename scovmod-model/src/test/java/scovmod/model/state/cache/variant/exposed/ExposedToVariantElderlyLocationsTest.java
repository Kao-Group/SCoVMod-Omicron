...
public class ExposedToVariantElderlyLocationsTest {
	...
	public void updatingByWholeLocationStatus(){
		ExposedToVariantElderlyLocations instance = new ExposedToVariantElderlyLocations();
		...
		assertEquals(NONE, instance.getExposedToVariantElderlyLocations());

		FakeLocalPopulation location2pop = new FakeLocalPopulation();
		location2pop.testSet(PERSON_5, EXPOSED_ELDERLY_VARIANT);
		instance.wholeLocationUpdate(LOCATION_2, location2pop);

		assertEquals(setOf(LOCATION_2), instance.getExposedToVariantElderlyLocations());

		FakeLocalPopulation location1updatedPop = new FakeLocalPopulation();
		location1updatedPop.testSet(PERSON_1, SUSCEPTIBLE);
		location1updatedPop.testSet(PERSON_3, EXPOSED_ELDERLY_VARIANT);
		instance.wholeLocationUpdate(LOCATION_1, location1updatedPop);

		assertEquals(setOf(LOCATION_2, LOCATION_1), instance.getExposedToVariantElderlyLocations());
	}
	...
	public void edgeCaseUpdatesDoNothing(){
		...
		FakeLocalPopulation location1pop = new FakeLocalPopulation();
		location1pop.testSet(PERSON_1, EXPOSED_ELDERLY_VARIANT);
		location1pop.testSet(PERSON_2, EXPOSED_YOUNG);
		location1pop.testSet(PERSON_3, EXPOSED_ADULT);
		instance.wholeLocationUpdate(LOCATION_1, location1pop);
		...
		assertEquals(setOf(LOCATION_1), instance.getExposedToVariantElderlyLocations());
	}
