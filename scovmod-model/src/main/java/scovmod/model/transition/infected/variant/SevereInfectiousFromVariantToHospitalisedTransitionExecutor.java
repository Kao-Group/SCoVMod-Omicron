...
public class SevereInfectiousFromVariantToHospitalisedTransitionExecutor {
    ...
    public SevereInfectiousFromVariantToHospitalisedTransitionExecutor(
            StateModifier sm,
            Random rnd,
            double timeStep,
    ...
    public void apply(int personID) {
        AgeClass ageClass = peopleAgeClasses.getOrDefault(personID,AgeClass.YOUNG);
            ...
            switch(ageClass){
                case YOUNG:
                    if (rnd.nextPoissonReturnsOneOrMore(params.getSiToH_YoungRate_covidVariant() * timeStep)) {
                        sm.updateInfectionState(personID, HOSPITALISED_YOUNG_VARIANT);
                    }
                    break;
                case ADULT:
                    if (rnd.nextPoissonReturnsOneOrMore(params.getSiToH_AdultRate_covidVariant() * timeStep)) {
                        sm.updateInfectionState(personID, HOSPITALISED_ADULT_VARIANT);
                    }
                    break;
                case ELDERLY:
                    if (rnd.nextPoissonReturnsOneOrMore(params.getSiToH_ElderlyRate_covidVariant() * timeStep)) {
