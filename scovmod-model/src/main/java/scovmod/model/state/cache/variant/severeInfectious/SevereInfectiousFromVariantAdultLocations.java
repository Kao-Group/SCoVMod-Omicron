...
import static scovmod.model.state.infection.InfectionState.SEVERE_INFECTIOUS_ADULT_VARIANT;

public class SevereInfectiousFromVariantAdultLocations implements Cache {

    IntSet locations = new IntOpenHashSet();

    public void wholeLocationUpdate(int locationId, LocalPopulation population) {
        if (!population.getAllInState(SEVERE_INFECTIOUS_ADULT_VARIANT).isEmpty()) {
