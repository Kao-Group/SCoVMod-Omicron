...
import scovmod.model.output.modules.dzAllStates.{DZAllStatesSummary, DZAllStatesSummaryResult,DailyResult}
...
case object FitAssessmentDZLevelAllStates extends Task {
  ...
  def runModel(runId: Int, particleWriterCSV: BufferedWriter, job: Job): Unit = {
    import job._
    val out = new DZAllStatesSummary;
    ...
    val dailySimDZResult = simulated.asInstanceOf[DZAllStatesSummaryResult]
    import scala.collection.JavaConverters._
    val simResult: util.Map[DailyResult, Integer] = dailySimDZResult.getSimulatedDailyResultPerDZ
