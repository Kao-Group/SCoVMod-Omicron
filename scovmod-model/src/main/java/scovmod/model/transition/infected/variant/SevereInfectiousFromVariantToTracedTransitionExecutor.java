...
public class SevereInfectiousFromVariantToTracedTransitionExecutor {
    ...
    public SevereInfectiousFromVariantToTracedTransitionExecutor(
            StateModifier sm,
            Random rnd,
            double timeStep,
    ...
    public void apply(int personID) {
        ...
        //Get what age group the person is in
            switch(ageClass){
                case YOUNG:
                    if (rnd.nextPoissonReturnsOneOrMore(tr.getTracingRate(params.getSiToT_YoungRate_covidVariant(),location)  * timeStep)) {
                        sm.updateInfectionState(personID, TRACED_YOUNG_VARIANT);
                    }
                    break;
                case ADULT:
                    if (rnd.nextPoissonReturnsOneOrMore(tr.getTracingRate(params.getSiToT_AdultRate_covidVariant(),location)  * timeStep)) {
                        sm.updateInfectionState(personID, TRACED_ADULT_VARIANT);
                    }
                    break;
                case ELDERLY:
                    if (rnd.nextPoissonReturnsOneOrMore(tr.getTracingRate(params.getSiToT_ElderlyRate_covidVariant(),location)  * timeStep)) {
