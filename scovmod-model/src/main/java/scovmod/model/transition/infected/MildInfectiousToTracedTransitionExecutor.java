...
public class MildInfectiousToTracedTransitionExecutor {
    ...
    public MildInfectiousToTracedTransitionExecutor(
            StateModifier sm,
            Random rnd,
            double timeStep,
            Parameters params,
    ...
    public void apply(int personID) {
        ...
        //Get what age group the person is in
            switch(ageClass){
                case YOUNG:
                    if (rnd.nextPoissonReturnsOneOrMore(tr.getTracingRate(params.getMiToT_YoungRate(),location) * timeStep)) {
                        sm.updateInfectionState(personID, TRACED_YOUNG);
                    }
                    break;
                case ADULT:
                    if (rnd.nextPoissonReturnsOneOrMore(tr.getTracingRate(params.getMiToT_AdultRate(),location) * timeStep)) {
                        sm.updateInfectionState(personID, TRACED_ADULT);
                    }
                    break;
                case ELDERLY:
                    if (rnd.nextPoissonReturnsOneOrMore(tr.getTracingRate(params.getMiToT_ElderlyRate(),location) * timeStep)) {
