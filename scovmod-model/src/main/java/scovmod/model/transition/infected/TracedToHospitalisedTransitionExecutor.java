...
public class TracedToHospitalisedTransitionExecutor {
    ...
    public TracedToHospitalisedTransitionExecutor(
            StateModifier sm,
            Random rnd,
            double timeStep,
            Parameters params,
    ...
    public void apply(int personID) {
        AgeClass ageClass = peopleAgeClasses.getOrDefault(personID,AgeClass.YOUNG);
            ...
            switch(ageClass){
                case YOUNG:
                    if (rnd.nextPoissonReturnsOneOrMore(params.gettToH_YoungRate() * timeStep)) {
                        sm.updateInfectionState(personID, HOSPITALISED_YOUNG);
                    }
                    break;
                case ADULT:
                    if (rnd.nextPoissonReturnsOneOrMore(params.gettToH_AdultRate() * timeStep)) {
                        sm.updateInfectionState(personID, HOSPITALISED_ADULT);
                    }
                    break;
                case ELDERLY:
                    if (rnd.nextPoissonReturnsOneOrMore(params.gettToH_ElderlyRate() * timeStep)) {
