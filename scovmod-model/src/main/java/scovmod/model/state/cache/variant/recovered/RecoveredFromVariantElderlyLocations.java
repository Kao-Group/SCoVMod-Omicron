...
import static scovmod.model.state.infection.InfectionState.RECOVERED_ELDERLY_VARIANT;

public class RecoveredFromVariantElderlyLocations implements Cache {

    IntSet locations = new IntOpenHashSet();

    public void wholeLocationUpdate(int locationId, LocalPopulation population) {
        if (!population.getAllInState(RECOVERED_ELDERLY_VARIANT).isEmpty()) {
